...
DEFAULT_FONT = "Times"  # Only 'Times', 'Courier', 'Helvetica', etc. are supported in FPDF
...
def plot_top_contributors(variance_df, metric, top_n=10):
    ...
    # Top positive, then top negative (reverse so max positive at top)
    top_pos = plot_df.head(top_n)
    top_neg = plot_df.tail(top_n).sort_values(by=col, ascending=False)
...
def generate_excel(variance_df, npv_column, filtered_wells_df, begin_df, final_df, nri_df):
    ...
    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
        ...
        for metric in MAIN_METRICS + [npv_column]:
            col = f"{metric} Variance"
            sheet = f"Top {metric} Contr." if len(f"Top {metric} Contr.") <= 31 else f"{metric[:27]}Contr."
...
def generate_pdf(variance_df, npv_column, explanation_df, nri_df):
    ...
    # 1. Write all summaries first (keeps all summary tables first)
    for category in categories:
        ...
        if not category_df.empty:
            major_changes = category_df[category_df[f"{npv_column} Variance"].abs() > category_df[f"{npv_column} Variance"].abs().quantile(0.95)]
            for _, row in major_changes.iterrows():
                ...
                if "Revenue" in metric or "$" in metric or "Capex" in metric or metric == npv_column:
                    val_str = f"${value:,.0f}"  # <-- NO DECIMALS for value change
                elif "Oil" in metric:
...
def compute_reports(begin_df, final_df, npv_column):
    ...
    excel_buffer = generate_excel(variance_df, npv_column, negative_npv_wells, begin_df_s, final_df_s, nri_df)
...
uploaded_begin = st.file_uploader("Upload BEGIN Excel file (.xlsx)", type=["xlsx"], key="begin_xls")
uploaded_final = st.file_uploader("Upload FINAL Excel file (.xlsx)", type=["xlsx"], key="final_xls")

npv_column = None
npv_col_choices = ["NPV at 9%", "NPV at 10%"]
if uploaded_begin and uploaded_final:
    begin_df = load_oneline(uploaded_begin)
    final_df = load_oneline(uploaded_final)
    npv_col_found = [col for col in npv_col_choices if col in begin_df.columns and col in final_df.columns]
    if npv_col_found:
        npv_column = st.selectbox("Select NPV column", npv_col_found)
    else:
        st.error("No NPV columns found in files. Make sure 'NPV at 9%' or 'NPV at 10%' exist.")

    if npv_column:
        if st.button("Generate & Download Reports"):
            with st.spinner("Generating reports..."):
                ...
                st.download_button(
                    ...
                    data=zip_buffer,
                    file_name=f"variance_reports_{datetime.now().strftime('%Y%m%d_%H%M')}.zip",
                    mime="application/zip"
                )
else:
    st.info("Please upload both BEGIN and FINAL Excel files to continue.")
